...
/// and depending on `A`.
public final actor SafeContainer {
   internal var services = [ServiceKey: ServiceEntryProtocol]()
   private let parent: SafeContainer? // Used by HierarchyObjectScope
   ...
   public init(
       ...
       defaultObjectScope: ObjectScope = .graph,
       behaviors: [Behavior] = [],
       registeringClosure: (SafeContainer) -> Void = { _ in }
   ) {
   ...
   public func withObjectGraph<T>(_ identifier: GraphIdentifier, closure: (SafeContainer) throws -> T) rethrows -> T {
...
extension SafeContainer: _Resolver {
   // swiftlint:disable:next identifier_name
...
extension SafeContainer: Resolver {
...
extension SafeContainer: CustomStringConvertible {
...
private extension SafeContainer {
